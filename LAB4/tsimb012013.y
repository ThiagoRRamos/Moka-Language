...
Prog		:	{InicTabSimb ();} PROGRAMA   ID   {InsereSimb($3, IDPROG, NOTVAR);} PVIRG  {printf ("programa %s ;\n", $3);}
				Decls CmdComp {ImprimeTabSimb (); VerificaInicRef();}
         ;
Decls 	:
			|	VAR  {printf ("var\n");}   ListDecl
         ;
...
ListElemDecl:	ElemDecl
         ...
         ;
ElemDecl :	ID  {if  (ProcuraSimb ($1)  !=  NULL)
					DeclaracaoRepetida ($1);
				else
					InsereSimb ($1,  IDVAR,  tipocorrente);} {printf ("%s ", $1);}
         ;
...
Fator		:  Variavel {if  ($1 != NULL){
         ...
         ;
Variavel	:  ID   {simb = ProcuraSimb ($1);
