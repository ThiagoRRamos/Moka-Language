...
ElemDecl  	:	ID
				{
					AnalisarInsercaoSimb($1, IDVAR, FALSE, escopoatual);}  |    ID   ABCOL {AnalisarInsercaoSimb($1, IDVAR, TRUE, escopoatual);}  ListDim   FCOL  {}  
...
CabFunc	:   	FUNCAO Tipo   ID 	ABPAR  FPAR  PVIRG {AnalisarInsercaoSimb ($3, IDFUNC, FALSE, escopoglobal);escopoatual=simb;}
		|		FUNCAO Tipo   ID ABPAR  { AnalisarInsercaoSimb ($3, IDFUNC, TRUE, escopoglobal);escopoatual=simb;} ListParam  FPAR PVIRG

CabProc	:   	PROCEDIMENTO   ID   ABPAR   FPAR   PVIRG {AnalisarInsercaoSimb ($2, IDPROC, FALSE, escopoglobal);escopoatual=simb;}
		|	PROCEDIMENTO   ID   ABPAR { AnalisarInsercaoSimb ($2, IDPROC, TRUE, escopoglobal);escopoatual=simb;} ListParam   FPAR   PVIRG

ListParam	:   	Parametro   |   ListParam   VIRG {}   Parametro

Parametro	:   	Tipo   ID {AnalisarInsercaoSimb($2, IDVAR, FALSE, escopoatual); $$ = simb; $$->inic = TRUE;  ConferirValorParam(tipocorrente);}

CmdComp	:     	ABCHAV  {tabular ();  tab++;} ListCmd   FCHAV {tab--; tabular (); }
...
CmdSe		:   	SE   ABPAR {tabular();tab++;}  Expressao {ConferirExpLogica($4);}  FPAR {}   Comando {tab--;}  CmdSenao

CmdSenao	:    	   |   SENAO {tabular();tab++;}  Comando {tab--;}

CmdEnquanto	:   	ENQUANTO   ABPAR {tabular(); }  Expressao {ConferirExpLogica($4);}  FPAR {tab++;}  Comando {tab--;}

CmdRepetir	:   	REPETIR {tabular();  tab++;}  Comando   ENQUANTO {tab--; tabular();  }  ABPAR   Expressao {ConferirExpLogica($7);}   FPAR   PVIRG

CmdPara	:   	PARA {tabular();tab++;} ABPAR Variavel {ConferirPara($4);InicializarEReferenciar($4);} ATRIB  Expressao    {ConferirAtribuicao($4, $7);} PVIRG {}  Expressao   PVIRG {ConferirExpLogica($11);}  Variavel   ATRIB { MesmaVariavel($4,$14);}   Expressao   FPAR {}   Comando {tab--;}

CmdLer	:   	LER   ABPAR  {tabular();} ListVar   FPAR   PVIRG

ListVar	:   	Variavel {InicializarEReferenciar($1);}  |   ListVar   VIRG {}   Variavel {InicializarEReferenciar($4);}
...
ListExpr	:   	Expressao {$$ = 1;checarTipo(1,$1);}  |   ListExpr   VIRG {}  Expressao {$$ = $$ + 1;checarTipo($$,$4);}
...
ListSubscr	: ExprAux4 {ConferirSubscr($1);$$ =1;} | ListSubscr   VIRG {}   ExprAux4 {ConferirSubscr($4);$$ =$1+1;}
