...
Prog		: 	{InicTabSimb ();} PROGRAMA   ID   PVIRG
				{printf("programa %s;\n", $3);
					...
					escopoglobal=escopoatual;}  Decls    SubProgs    CmdComp
				{ImprimeTabSimb ();
					VerificaInicRef();}
...
ElemDecl  	:	ID
				{printf("%s", $1);
					AnalisarInsercaoSimb($1, IDVAR, FALSE, escopoatual);}  |    ID   ABCOL {printf("%s[", $1);AnalisarInsercaoSimb($1, IDVAR, TRUE, escopoatual);}  ListDim   FCOL  {printf("]");}  
...
CabFunc	:   	FUNCAO Tipo   ID 	ABPAR  FPAR  PVIRG {printf("funcao %s %s();\n", $2, $3); AnalisarInsercaoSimb ($3, IDFUNC, FALSE, escopoglobal);escopoatual=simb;}
		|		FUNCAO Tipo   ID ABPAR  {printf("funcao %s %s(", $2, $3); AnalisarInsercaoSimb ($3, IDFUNC, TRUE, escopoglobal);escopoatual=simb;} ListParam  FPAR PVIRG {printf(");\n");}

CabProc	:   	PROCEDIMENTO   ID   ABPAR   FPAR   PVIRG {printf("procedimento %s();\n", $2); AnalisarInsercaoSimb ($2, IDPROC, FALSE, escopoglobal);escopoatual=simb;}
		|	PROCEDIMENTO   ID   ABPAR {printf("procedimento %s(", $2); AnalisarInsercaoSimb ($2, IDPROC, TRUE, escopoglobal);escopoatual=simb;} ListParam   FPAR   PVIRG {printf(");\n");}
...
CmdSe		:   	SE   ABPAR {tabular();tab++;printf("se(");}  Expressao {ConferirExpLogica($4);}  FPAR {printf(")\n");}   Comando {tab--;}  CmdSenao

CmdSenao	:    	   |   SENAO {tabular();tab++;printf("senao\n");}  Comando {tab--;}

CmdEnquanto	:   	ENQUANTO   ABPAR {tabular(); printf("enquanto (");}  Expressao {ConferirExpLogica($4);}  FPAR {printf(")\n");tab++;}  Comando {tab--;}

CmdRepetir	:   	REPETIR {tabular(); printf("repetir \n"); tab++;}  Comando   ENQUANTO {tab--; tabular(); printf("enquanto("); }  ABPAR   Expressao {ConferirExpLogica($7);}   FPAR   PVIRG {printf(");\n"); }

CmdPara	:   	PARA {tabular();tab++;printf("para(");} ABPAR Variavel {printf(" := ");ConferirPara($4);InicializarEReferenciar($4);} ATRIB  Expressao    {ConferirAtribuicao($4, $7);} PVIRG {printf("; ");}  Expressao   PVIRG {printf("; ");ConferirExpLogica($11);}  Variavel   ATRIB {printf(" := "); MesmaVariavel($4,$14);}   Expressao   FPAR {printf(")\n");}   Comando {tab--;}

CmdLer	:   	LER   ABPAR  {tabular();printf("ler(");} ListVar   FPAR   PVIRG {printf(");\n");}

ListVar	:   	Variavel {InicializarEReferenciar($1);}  |   ListVar   VIRG {printf(", ");}   Variavel {InicializarEReferenciar($4);}
...
CmdRetornar	:   	RETORNAR   PVIRG {tabular (); printf("retornar;\n");ConferirRetornoProcedimento(escopoatual);} 
			|   RETORNAR {tabular(); printf("retornar ");}  Expressao  {ConferirRetornoFuncao(escopoatual,$3);} PVIRG {printf(";\n");}
CmdAtrib  	:   	{tabular ();} Variavel {if  ($2 != NULL) $2->inic = $2->ref = TRUE;printf(" := ");}   ATRIB  Expressao   PVIRG {printf(";\n"); ConferirAtribuicao($2, $5);}
ListExpr	:   	Expressao {$$ = 1;checarTipo(1,$1);}  |   ListExpr   VIRG {printf(", ");}  Expressao {$$ = $$ + 1;checarTipo($$,$4);}
...
Variavel	:   	ID {printf("%s",$1);
					...
					if (simb == NULL)   NaoDeclarado ($1);
         			...
         			$<simb>$ = simb;
	}   ListSubscr   FCOL   { printf ("] ");  $$ = $<simb>3;
		if ($$ != NULL)
			if ($$->array == FALSE)
				NaoEsperado ("Subscrito\(s)");
                 	 else if ($$->ndims != $4)
 				Incompatibilidade ("Numero de subscritos incompativel com declaracao");}
ListSubscr	: ExprAux4 {ConferirSubscr($1);$$ =1;} | ListSubscr   VIRG {printf(",");}   ExprAux4 {ConferirSubscr($4);$$ =$1+1;}
...
void ConferirAtribuicao(simbolo p, int q){
	if (p != NULL){
		if (((p->tvar == INTEGER || p->tvar == CHAR) && (q == FLOAT || q == LOGICAL)) || (p->tvar == FLOAT && q == LOGICAL) || (p->tvar == LOGICAL && q != LOGICAL)){
				printf("%s %s",nometipvar[p->tvar],nometipvar[q]);
